...
st.set_page_config(page_title="BUSINESS Pillar", page_icon=":office:", layout="wide")
...
st.title(":office: BUSINESS Pillar - Stocks")
...
tab1, tab2 = st.tabs([":chart_with_upwards_trend: Stock Holdings", ":telescope: Watchlist"])
...
with tab1:
      ...
      if business_holdings:
                ...
                st.info(":bulb: **BUSINESS Strategy**: Focus on value/growth names with strong moats.")
else:
        st.warning("No stock holdings yet. Add your first stock in the 'Add Data' page!")

with tab2:
      st.subheader("Watchlist")
      st.info(":telescope: **Research Area**: Keep track of businesses you'd buy if the price was right.")
      watchlist_data = [
...
if business_holdings:
      ...
      fig.update_layout(title="Stock Allocation within BUSINESS", height=400)
