...
if 'db' not in st.session_state:
    # Pull DATABASE_URL from Streamlit secrets if available (cloud deployment)
    try:
        import os
...
with st.sidebar:
    ...
    if st.button("🔄 Check Rebalance", use_container_width=True):
...
st.subheader("🎯 Pillar Allocation (Target: 50-30-20)")
...
pillars_data = [
    ('LIQUID', '💧', 'liquid-card', col1, portfolio['liquid_value'], portfolio['liquid_pct']),
    ('BUSINESS', '🏢', 'business-card', col2, portfolio['business_value'], portfolio['business_pct']),
    ('LAND', '🏠', 'land-card', col3, portfolio['land_value'], portfolio['land_pct'])
]
...
# Footer
st.markdown("---")
st.caption("💡 Tip: Use the sidebar to navigate between different views and add new data.")
